// ============================================================
// article.js - VERSION COMPLÈTE AVEC PARSING DES COLONNES DYNAMIQUES
// Structure: -s- slide, -q- question quiz, -v- bonne réponse,
// -f- mauvaise réponse, -qs- question ouverte, -r- retenir, -i- info
// ============================================================
...
// ============================================================
// PARSER LE CONTENU STRUCTURÉ (NOUVELLE LOGIQUE)
// ============================================================
function parserContenuStructure(contenu) {
  var result = {
    slides: [],
    slidesIdee: [],
    quizs: [],
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (ligne.startsWith('-s-')) {
      result.slides.push(ligne.replace('-s-', '').trim());
      result.slidesIdee.push(false);
    } else if (ligne.startsWith('-q-')) {
      var qText = ligne.replace('-q-', '').trim();
      var reponses = [];
      // Chercher les réponses associées (qui suivent immédiatement)
      for (var j = i + 1; j < elements.length; j++) {
        var nextLine = elements[j].trim();
      ...
      result.quizs.push({ question: qText, reponses: reponses });
    } else if (ligne.startsWith('-qs-')) {
      result.openQuestions.push(ligne.replace('-qs-', '').trim());
      ...
      result.infos.push(ligne.replace('-i-', '').trim());
    }
  }
...
function afficherArticle(post) {
  ...
  var contenuParse = parserContenuStructure(post.contenu || '');
  
  var slides = [];
  
  // 1. Ajouter les slides
  contenuParse.slides.forEach(function(slide, index) {
    slides.push({ type: 'text', data: { contenu: slide }, estIdee: contenuParse.slidesIdee[index] || false });
  });
  
  // 2. Ajouter les quiz
  hasQuiz = contenuParse.quizs.length > 0;
  if (hasQuiz) {
    slides.push({ type: 'quiz', data: { questions: contenuParse.quizs } });
  }
  
  // 3. Ajouter les questions ouvertes
  hasQuestion = contenuParse.openQuestions.length > 0;
  if (hasQuestion) {
    contenuParse.openQuestions.forEach(function(q) {
      slides.push({ type: 'question-ouverte', data: { question: q } });
    });
  }
  
  // 4. Ajouter "À retenir"
  if (contenuParse.retenir.length > 0) {
    slides.push({ type: 'memo', data: { texte: contenuParse.retenir.join(', ') } });
  }
  
  // 5. Ajouter les infos (optionnel)
  if (contenuParse.infos.length > 0) {
    contenuParse.infos.forEach(function(info) {
      slides.push({ type: 'text', data: { contenu: 'ℹ️ ' + info }, estIdee: false });
    });
  }
  
  // 6. Si aucune slide, ajouter une slide par défaut
  if (slides.length === 0) {
    slides.push({ type: 'text', data: { contenu: 'Contenu non disponible.' }, estIdee: false });
  }
  ...
  slides.forEach(function(slide, index) {
    ...
    if (slide.type === 'quiz') {
      ...
      div.innerHTML = genererSlideTexte(slide.data, slide.estIdee);
    }
...
// ============================================================
function genererSlideTexte(data, estIdee) {
  ...
  var ideeClass = estIdee ? ' slide-idee' : '';
  return '<div class="contenu-slide' + ideeClass + '">' + contenuArabe + '</div>';
}
...
function genererSlideQuiz(data) {
  ...
  data.questions.forEach(function(q, idx) {
    ...
    q.reponses.forEach(function(rep, ri) {
      var isCorrect = rep.correct ? 'data-correct="true"' : '';
      html += '<label>';
      html += '<input type="radio" name="' + qId + '" value="' + rep.texte + '" ' + isCorrect + '>';
      html += lettres[ri] + '. ' + rep.texte;
      html += '</label>';
    });
...
function ajouterBoutonAppris() {
  // Fonction vide car le bouton est déjà dans la slide memo
}
